...
# have a value greater than zero, that pixel is turned 'ON'.
# i.e. set to 255 in the output image. If both pixels are not
# greater than zero, then output pixel is left 'OFF', with a
# value of 0.
...
# If either pixel in rectangle or circle is greather than zero, then
# the output pixel is value of 255, otherwise it is 0.
bitwiseOr = cv2.bitwise_or(rectangle, circle)
cv2.imshow('OR', bitwiseOr)
cv2.waitKey(0)

# The bitwise 'XOR' is identical to the 'OR' function,
# with one exception: both rectangle and circle are not allowed
# to both have values greater than 0.
