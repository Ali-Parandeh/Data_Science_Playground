...
# It is also possible to go the other way around and push local 
# CSV files back to the database. As long as we specify the database 
# as well as the CSV file to be loaded, csvsql does the rest of the 
# work for us (e.g. inferring table schema), behind the scenes.

# In the following exercise, complete the command to upload 
# Spotify_MusicAttributes.csv as its own table in the SQLite 
# database SpotifyDatabase. Then, as a sanity check, re-pull 
# the data from the newly created table in the database.

# 1. Upload Spotify_MusicAttributes.csv as its own table in the SQLite database SpotifyDatabase.
# 2. Re-pull the data from the newly created table Spotify_MusicAttributes in the SQLite database SpotifyDatabase.
...
# csvsql makes the upload look so easy. However, if you 
# do get unexpected errors, you can add in the --verbose 
# flag to your command to print out more error logs.
